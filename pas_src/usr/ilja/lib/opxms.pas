...
{*********************************************************}
{$IFDEF DPMI}
	This code is not compatible with DPMI
{$ENDIF}
...
type
  {pointers in XMS are segm:ofs for < 1 meg, and linear for > 1 meg}
	...
	{the record structure used for accessing shared XMS blocks }
	ExtMemHand =
		record
			Hand: Word;
			Len: Longint; { length of part }
			Ofs: Longint; { offset in XMS block to part }
		end;
...
function ReadFileToXMS(var F : file; var Handle : Word; Count : Longint) : Integer;

	{==========================================================================}

implementation

uses Crt, ErrTxt, FileExt, IOErrDef, Math, MemExt;
...
var
	MoveParams: ExtMemMoveStruct; {The XMS driver doesn't like this on the stack}
...
var
	...
	procedure MakeErrorText;
	begin
		AET := 'ReadFileToXMS error'+' ('+XMSErrorString(Result)+').';
	end;

begin
	ReadFileToXMS := hError;
	Result := QueryFreeExtMem(TotalFree, LargestBlock);
	if Result <> 0 then begin MakeErrorText; Exit end;
	if LargestBlock < (Count + 1023) div 1024 then begin AET := AET+' (insufficient extended memory).'; Exit end;
	Result := AllocateExtMem((Count + 1023) div 1024, Handle);
	if Result <> 0 then
		begin MakeErrorText; Exit; end;
	MoveParams.DestOffs.ProtectedPtr := 0;
	if MaxFree < SizeOf(Buf) then begin AET := NoMemAlloc+'file buffer'; Exit end;
	New(Buf);
	MoveParams.SrcHand := 0;
	MoveParams.SrcOffs := ExtMemPtr(Buf);
	MoveParams.DestHand   := Handle;
	Remaining := Count;
	while Remaining > 0 do
	begin
		MoveParams.Len := Min(Remaining, BufferLen);
		if BlockRead(F, Buf^, MoveParams.Len) <> hOk then
			begin Dispose(Buf); Exit end;
		MoveParams.Len := ParL(MoveParams.Len);
	{XMS copy lengths must be a multiple of two}
		with MoveParams do
			Result := MoveExtMemBlock(Len, SrcHand, SrcOffs, DestHand, DestOffs);
		if Result <> 0 then
			begin MakeErrorText; Dispose(Buf); Exit end;
		Inc(MoveParams.DestOffs.ProtectedPtr, MoveParams.Len);
		Dec(Remaining, MoveParams.Len);
	end;
	Dispose(Buf);
	AET := '';
	ReadFileToXMS := hOk;
...
begin
	if Lo(DosVersion) >= 3 then begin               {!!.12}
		...
		XmsControl := Nil;                            {!!.12}
	Moving := False;
{	WriteLn(Seg(XmsControl^),':',Ofs(XmsControl^));
	ReadKey;}
end.